...
def show_Kecamatan(path, w=12, h=8):
    ...
    for x0, y0 in path:
        plt.plot(x0, y0, 'y*', markersize=5)  # y* -> Titik start        
    plt.axis("off")
    fig = plt.gcf()
    fig.set_size_inches([w, h])
    plt.savefig('Kecamatan.png')  
    plt.show()  # Menampilkan Figure

def show_path(path, starting_kecamatan=None, w=12, h=8):
    """Plot a TSP path overlaid on a map of the Kantor Kecamatan in Surabaya."""
    if isinstance(path, dict):      path = list(path.values())
    if isinstance(path[0][0], str): path = [ item[1] for item in path ]
...
Kecamatan = {
    ...
    "Sukolilo": (606, 521),
    "Rungkut": (613, 595),
    "Gunung Anyar": (640, 651),
    "Tenggilis Mejoyo": (561, 570),
    "Wonocolo": (517, 587),
    "Gayungan": (443, 639),
    "Jambangan": (435, 597),
    "Karang Pilang": (391, 625),
    "Wiyung": (378, 574),
    "Dukuh Pakis": (418, 460),
    "Lekarsantri": (227, 552),
    "Sawahan": (438, 486),
    "Wonokromo": (488, 501),
    "Tegalsari": (511, 485),
    "Sukomanunggal": (421, 431),
    "Sambikerep": (244, 464),
    "Tandes": (328, 403),
    "Asemrowo": (434, 381),
    "Benowo": (207, 383),
    "Pakal": (172, 350),
    "Pabean Cantian": (484, 269)
}
